...
class TechnicianSkillController extends Controller
{
    ...
    /**
     * Get skill match scores for a ticket
     */
    public function getMatchScores(Request $request, int $ticketId)
    {
        $ticket = \App\Models\Ticket::with('device.deviceType')->findOrFail($ticketId);
        
        if (!$ticket->device || !$ticket->device->device_type_id) {
            return response()->json([
                'message' => 'Ticket does not have a device type assigned',
            ], 422);
        }

        $technicians = Technician::where('status', 'on_duty')
            ->with(['skills.deviceType', 'user'])
            ->get();

        $skillMatchingService = app(\App\Services\Workflow\SkillMatchingService::class);

        $scores = $technicians->map(function ($technician) use ($ticket, $skillMatchingService) {
            $score = $skillMatchingService->calculateMatchScore($technician, $ticket);
            $skill = $technician->getSkillForDeviceType($ticket->device->device_type_id);
        ...
        })->sortByDesc('skill_match_score')->values();

        return response()->json([
            'ticket_id' => $ticketId,
            'device_type' => $ticket->device->deviceType->name,
            'match_scores' => $scores,
        ]);
    }
}
