...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h3 class="text-2xl font-bold text-slate-800">Customer Analytics Report</h3>
            <p class="text-sm text-slate-500 mt-1">Customer lifetime value and spending patterns</p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-slate-100 p-6">
        <form method="GET" action="{{ route('admin.reports.customer') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
    ...
    <!-- Report Table -->
    <div class="bg-white rounded-2xl shadow-xl border border-slate-100 p-6">
        <h4 class="text-lg font-bold text-slate-800 mb-4">Top Customers by Lifetime Value</h4>
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-slate-200">
                        ...
                        <th class="px-4 py-3 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Spent</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                <div class="h-10 w-10 rounded-full bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center text-white font-bold">
                                    {{ substr($item['customer']->name, 0, 1) }}
                                </div>
                                <div>
                        ...
                        <td class="px-4 py-3 text-sm text-right font-mono font-semibold text-slate-800">@currency($item['total_spent'])</td>
                        <td class="px-4 py-3 text-sm text-right text-slate-600">{{ $item['transactions'] }}</td>
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-4 py-8 text-center text-slate-500">No customer data found for the selected period</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
